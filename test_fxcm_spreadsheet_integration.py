...
class FXCMSpreadsheetIntegrationTest:
        ...
        try:
            ...
            if prices:
                ...
                for symbol, price_data in prices.items():
                    logger.info(f"  {symbol}: Bid={price_data['bid']}, Ask={price_data['ask']}, Spread={price_data['spread']:.5f}")
                return True
            else:
        ...
        try:
            ...
            if account_summary:
                ...
                if 'account_info' in account_summary:
                    ...
                    logger.info(f"  Balance: {account_info.get('balance', 0)} {account_info.get('currency', 'USD')}")
                    logger.info(f"  Equity: {account_info.get('equity', 0)}")
                
                logger.info(f"  Total Positions: {account_summary.get('total_positions', 0)}")
        ...
        for i, symbol in enumerate(symbols[:3]):  # Generate signals for first 3 symbols
            if symbol in prices:
                ...
                signal = {
                    ...
                    'StopLoss': price_data['ask'] - 0.005 if i % 2 == 0 else price_data['bid'] + 0.005,
                    'TakeProfit': price_data['ask'] + 0.010 if i % 2 == 0 else price_data['bid'] - 0.010,
    ...
    async def test_spreadsheet_output(self):
        ...
        try:
            ...
            if created_files:
                logger.info(f"✓ Spreadsheet output test successful - {len(created_files)} files created")
                return True
            else:
    ...
    async def test_market_data_to_spreadsheet_flow(self):
        ...
        try:
            ...
            if self.config["spreadsheet"]["include_market_data"]:
                ...
                logger.info(f"✓ Market data updated in spreadsheet ({len(market_data_signals)} symbols)")
            ...
            if self.config["spreadsheet"]["include_account_info"] and account_summary:
                account_signal = {
                    ...
                    'Symbol': 'ACCOUNT',
                    'Action': 'INFO',
                    'Balance': account_summary.get('account_info', {}).get('balance', 0),
